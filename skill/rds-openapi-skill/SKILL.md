---
name: rds-openapi-skill
description: 通过阿里云 RDS OpenAPI 与只读 SQL 工具管理 RDS 实例。在用户需要查询/管理阿里云 RDS、执行只读 SQL、查看实例/监控/慢日志/参数时使用。需先配置环境变量 ALIBABA_CLOUD_ACCESS_KEY_ID、ALIBABA_CLOUD_ACCESS_KEY_SECRET，再通过命令行工具 rds-openapi-skill 列出并执行工具。
...
1. **安装**：已安装本包（任选其一）
   ...
   - 或使用：`uvx alibabacloud-rds-openapi-mcp-server` 时，需在本地安装后使用 `rds-openapi-skill` 命令
...
**OpenClaw**：在 `~/.openclaw/openclaw.json` 的 `skills.entries` 中为 `rds-openapi-skill` 增加一项，并写入 `env`：
...
**Claude Code / 其他平台**：在对应技能的配置中找到「entries」或「环境变量」项，为 `rds-openapi-skill` 配置上述两个 env 即可。
...
2. **执行工具**  
   ```bash
   rds-openapi-skill run <工具名> '<JSON 参数>'
   ```  
   ...
   - 示例：执行只读 SQL  
     ```bash
     rds-openapi-skill run query_sql '{"region_id":"cn-hangzhou","db_instance_id":"rm-xxxxx","db_name":"mydb","sql":"SELECT 1"}'
     ```
